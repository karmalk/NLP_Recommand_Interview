...
支持向量机是一个二分类模型，它的基本模型定义为特征空间上的**间隔最大的线性分类器**，因为支持向量机引入了核技巧，这使他成为实质上的非线性分类器，学习策略为**最大化分类间隔**，最终可形式化为**凸二次规划问题求解**，也等价于正则化的合页损失函数的最小化问题，支持向量机的学习算法是求解凸二次规划的最优化算法。

支持向量机的学习方法包含从简至繁的模型，包括有**线性可分支持向量机，线性支持向量机，非线性支持向量机。**
...
- **硬间隔SVM（线性可分SVM)**： 当训练数据可分时，通过间隔最大化，学习一个线性分类器。
...
常见的核函数包括，线性核 高斯(RBF)核，多项式核，sigmoid核函数等等
...
核函数能够将特征从低维空间映射到高维空间， 这个映射可以把低维空间中不可分的两类点变成线性可分的。核函数包括，线性核 高斯(RBF)核，多项式核，sigmoid核函数等等
...
- 由于SVM是一个**凸优化**问题，所以求得的解一定是全局最优解
